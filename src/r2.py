...
def identificar_comunidades_hubs(grafo, valorhub):
    # Identificar hubs
    degree_centrality = netx.degree_centrality(grafo)
    hubs = [node for node, centrality in degree_centrality.items() if centrality >= valorhub]  # valorhub e configuravel

    return hubs

def plotar_comunidades(G, comunidade):
    ...
    for i, comunidade in enumerate(comunidade, start=1):
        ...
        pos = netx.fruchterman_reingold_layout(comunidade_grafo)
...
def main():
    ...
    aresta = input("digite o nome do arquivo de arestas:")
    target = input("digite o nome do arquivo de target:")
    valorhub = float(input("Digite o valor do hub: "))
    
    grafo = ler_grafo(aresta, target)

    result_comunidade, modularidade = louvain_algorithm(grafo)

    hubs = identificar_comunidades_hubs(grafo,valorhub)
    
    pontes_comunidades = identifica_pontes_comunidades(grafo, result_comunidade)
    
    plotar_comunidades(grafo, result_comunidade)
    ...
    print()
    print("Os arquivos resultados.txt e grafo.png foram gerados, fique a vontade para visualizar os dados encontrados")\
